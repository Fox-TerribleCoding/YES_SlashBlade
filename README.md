...
TLM 丢的**三处**里，前两处是渲染。这里按 TLM 1.20/1.21 分支的源码**原样补回**那两处渲染分支
（变换数值一个没改）；第三处"挥刀"是行为，见 **§3.5**：
...
**1.0.16 打磨：出鞘 / 收鞘不再生硬跳变。**（**有意添加的增强**，非还原上游）

TLM 给女仆的"动作"本身只有 5 刻恒速自转（`rotY(60 + i*48)`），且是**二值**的：
窗口内加满、窗口一过整段消失 ⇒ 进出各"啪"一下。本模组在窗口**两端各留一小段缓动**
（默认 1 刻 = 50 ms），从"收鞘"姿态**球面插值**到出鞘姿态、位移同步缩放；
**窗口中段权重恒为 1，与原实现逐位相同** —— 只磨两头。

- 开关 `maidBladeEaseTicks`（**热重载**，默认 `1.0`；**`0` = 完全回到 TLM 原样**）
- ⚠️ **只抹掉跳变**，不改变上游那段转动；"成套剑技动作"不在本模组范围内（见 §6.2）
...
> ⚠️ **`require = 0` 只保护"靶点找不到"，不保护"靶点写法校验失败"。**
> 后者会让**整份混入类**一起作废（1.0.9 实测翻过车：一条失败的重定向让整个混入失效，
> 表现为"改了跟没改一样"，日志里只有一条很容易被忽略的 WARN）。
> 所以：**一个混入类里只放能过校验的靶点**（优先只碰 Minecraft / JOML 自己的类），
> 高风险靶点另开一个混入类；并且**每个混入类都要有一条"我还活着"的自检日志**。

### 3.7 ⑤ 第一人称的姿态基准（开光影时的刀位置）

**现象**：只要启用任意光影（不限种类），第一人称的刀就摆到别处，走动时还会在画面上漂移；
关闭光影则一切正常。**与光影包的类别无关**，也不涉及 BSL／Advanced Materials。

**根因**（逐条由字节码确证）：

1. 一个顶点最终的变换是 `ModelViewMat × 姿态栈`；
2. 无光影时 `GameRenderer#renderItemInHand` 令 **姿态栈 = 相机旋转的逆**、
   **`ModelViewMat` = 相机旋转**（两者互补）；
3. 拔刀剑的第一人称渲染会把姿态栈**清零**（`me.pose().identity()`）——
   清零之后最终变成 `相机旋转 × 刀变换`，**刀"跟着视角"正是靠这一层相机旋转**；
...
代码里另有一道**结构性保险**：先比较 `ModelViewMat` 与相机旋转，**相等就直接按原样清零** ——
所以"无光影零改动"不依赖等式在数值上恰好成立。

- 开关 `firstPersonPoseBase`（**热重载**，默认 `camera-view`）
- 本修正**不检测光影包、不判断光影种类**；未安装 Iris 时它完全惰性
- 附带说明：修复前"开光影时刀会晃"的观感**是 bug 的表现**（手部的视角摇晃矩阵漏到了刀上），
  修复后不再出现，与无光影、与 1.20.1 一致

**1.0.15 补修：同一处病根的另一半。**

- **现象**：开光影时，刀的水平方向**被锁死**（视角左右转、刀不跟着转），
  而竖直方向**随视角反着动**；正常（无光影）竖直方向根本不该动。
- **根因**：拔刀剑 2.0.7 自带一段「Iris 近似补偿」，用 yaw/pitch 去**近似**抵消 Iris 那层矩阵。
  其中 `rotY(yaw+180)` 与随后的 `rotY(180−yaw)` 抵消成 360°（**偏航整个归零**），
  而 `rotX(+xRot)` 与后面的 `rotX(−clamp)` 并不抵消（**多出一层俯仰**）。
  本模组既已精确处理 `ModelViewMat`，这段近似就成了纯错误。
- **修法**：不去拦那段分支（曾试过拦 `Iris.isPackInUseQuick()`，在 Mixin **校验阶段**就失败、
  还连累整份混入类作废），而是在拔刀剑的**构造器**里把 `ModList.isLoaded("iris")` 对它
  **隐瞒为 false** —— 靶点是 NeoForge 自己的 `ModList`，与"那段分支长什么样"完全解耦；
  **不碰 Iris、不碰光影包、不碰任何第三方文件**。
- 开关 `firstPersonIrisHack`（`auto` 默认 / `keep` 对照 / `ignore`）。
  ⚠️ **这一项需重启游戏才生效**（拔刀剑在首次构造时读一次该字段），与其它热重载开关不同。

### 3.8 ⑥ 第一人称刀的视角摇晃（**增强，非还原**）

原版第一人称的手会随脚步摇晃；而拔刀剑的第一人称渲染清零了姿态栈，摇晃正刻在被抹掉的那份
矩阵里 —— 于是**手在晃、刀不晃**。本项把那份摇晃补回到刀上。

- **直接调用原版的 `GameRenderer#bobView`**（经一个 `@Invoker` 混入），**不重写那几行数学**：
  与手部**逐像素一致**，且 Minecraft 日后改动摇晃算法时本模组**自动跟随**。
- **绑定游戏内置的「视角摇晃」开关**（`options.bobView()`）：玩家关掉它，刀也不晃。
- 插入位置在姿态基准**之后**：
  `姿态栈 := ModelViewMat⁻¹ × 相机旋转 × 摇晃` ⇒ 效果是**整把刀随视角摇**，
  而不是绕刀自身原点自转；有/无光影结果一致（与 §3.7 同源，化简后都是 `相机旋转 × 摇晃 × 刀变换`）。
- 开关 `firstPersonBladeBob`（默认 `true`，**热重载**）。

> ⚠️ **这是本模组有意添加的增强，不是"还原 1.20.1"。**
> 1.20.1 的第一人称刀**是不晃的**（它同样清零了姿态栈）。不需要就设 `firstPersonBladeBob=false`。

> **隔离说明**：`bobView` 是**私有**方法，名字与签名跨版本可能变，因此本项放在**独立的
> 混入类 + 独立混入配置**（`yessb.bob.mixins.json`，`required=false` + `defaultRequire=0`）里。
...
> 🔴 **重要（1.0.14 修）：1.0.5 ~ 1.0.13 在「没装车万女仆」的整合包里，左键一挥就崩游戏**（例如 ATM10）。
> 女仆挥刀钩子挂在原版挥击上，而它引用了车万女仆的类 —— 那个类不存在时，游戏在**加载阶段**就失败，
> `try/catch` 兜不住。**请升级到最新版本**（1.0.14 起已修复），换新版即可，不需要改任何配置。

把 `YES_SB-1.0.16.jar` 放进 `.minecraft/mods/`
...
| [车万女仆](https://modrinth.com/mod/touhou-little-maid) | 1.5.x | 补回它丢失的女仆拔刀剑**渲染与挥刀**分支 |
...
首次启动生成 `config/yes_sb.properties`。**几乎所有键都支持热重载** —— 唯一例外是 `firstPersonIrisHack`（见 §3.7），它需要重启游戏才生效。
...
| `firstPersonPoseBase` | `camera-view` | 第一人称刀的姿态基准（见 §3.7）。`camera-view` = 修正版；`identity` = 拔刀剑原行为；`modelview` = 仅供对照 |
| `firstPersonBladeBob` | `true` | 第一人称的刀是否跟随**视角摇晃**（见 §3.8）。跟随游戏内置的「视角摇晃」开关；**这是增强，1.20.1 没有** |
...
| `maidBladeEaseTicks` | `1.0` | 出鞘 / 收鞘两端的缓动长度（刻）。**`0` = 完全回到 TLM 原样（二值跳变）** |
...
**1.0.12 修掉了"开光影后第一人称的刀跑到别处、并随移动漂移"**（见 §3.7）。
该问题与光影包的类别无关，任何光影只要启用就会出现；本模组不检测光影包，
修的是"姿态基准"本身，因此对任意光影同样成立。
...
靶向注入大多依赖第三方模组的内部结构（YSM 的类经过混淆；车万女仆的两处渲染层同理）。
**上游更新后这些靶点极可能失效**，届时对应功能会自动跳过（表现为功能不生效，不会崩溃），
需要按新版本更新白名单或靶点。

> 唯一的例外是女仆的**挥刀**那一处（§3.5）：它挂的是**原版** `LivingEntity#swing`，
> 不依赖任何第三方内部结构 —— 它对上游版本的敏感度只体现在"上游哪天自己把覆写加回来时，
> 让路闸会自动生效"。

### 6.6 空中连招第二段收招后模型卡住 —— **非本模组**（已排除）

**现象**：空中连招（右键连按）**第二段**收招后，模型停在**滞空姿势**，
落地也不切回地面动画；**移动、疾跑、跳跃都无法解除**，只有按一次鼠标左/右键才恢复。
其余连段落地后都能正常切回。

**这不是本模组造成的**，排除过程如下（全部为实测）：

| 测试 | 结果 |
|---|---|
| 本模组两条动画注入**全部关闭** | **不复现** —— 但那时也没有任何拔刀剑专属动画 |
| 只开 `slashbladeComboAnimations`（剑技动画名） | 复现 |
| 只开 `slashbladeMainStateAnimations`（主动画变体） | 复现 |
...
> 所有靶向注入都带**版本白名单 + failure-soft**：上游一改靶点，对应功能**自动跳过**
> （表现为不生效，不会崩溃）⇒ 退役**不存在紧急时间窗**，可以慢慢做。
> 反过来也要注意：**上游修好而我们没退役时，用户可能分不清"是谁修好的"** —— 所以声明要写清楚。
...
| 本模组 | `YES_SB-1.0.16.jar` | 150,718 B | `194834F0A969D2B4` |

> **关于本模组这一行的哈希**：jar 内各条目带**构建时刻的时间戳**，所以**每次重新构建哈希都会变**
> （哪怕源码一字未改）。它只用于标识**某一次具体构建**，不是可以反复比对的恒定值 ——
> 真正稳定的对照基准是上面三个第三方 jar。
> **提交 issue 时请附上这张表**，可以立刻排除"版本不对"这一类干扰。
