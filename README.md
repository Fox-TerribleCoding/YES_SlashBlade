...
把 `YES_SB-1.0.0.jar` 放进 `.minecraft/mods/`
...
| 键 | 默认 | 作用 |
|---|---|---|
| `firstPersonAsHeldItem` | `true` | 第一人称按"手持物"渲染，而不是拔刀剑默认那套"挂在身体上的整套刀+鞘" |
| `firstPersonMode` | `model` | `model` = 画 3D 刀身 / `icon` = 画平面图标 / `off` = 不干预（回到拔刀剑原本行为） |
| `firstPersonScale` | `0.0062` | 3D 刀身缩放 |
| `firstPersonRotX/Y/Z` | `0` | 刀身朝向（角度） |
| `firstPersonOffsetX/Y/Z` | `0` | 刀身位移（方块） |
...
第一人称的缩放 / 朝向 / 位移都在调参中。这几个上下文现在各有独立参数，
可以直接改配置热重载试，不需要重新编译。
...
本模组的定位是**临时补丁**，不是替代品。
下面两处问题的反馈稿已整理完毕，**将在本模组发布后一并提交给对应上游**。

### 车万女仆（TLM）

§2.4 那两处**是发布版的移植遗漏**：源码在、发布 jar 里没有，可确证。
本模组只是把它补回去。**上游修复后，本模组的女仆部分即可退役。**

### Yes Steve Model（YSM）

YSM 侧拔刀剑兼容模块被掏空的**原因尚未确认** ——
2.6.5 的更新日志里有「在联动模组版本过低时自动禁用联动功能（而不是报错退出）」这样的条目，
所以**不排除是有意停用**。已通过官方渠道询问，等待答复。

若官方回复"是有意停用"，则 §6.1 提到的"绑定骨骼"方案**应当放弃**，
本模组也不会朝那个方向继续做。
...
| [Yes Steve Model](https://modrinth.com/mod/yes-steve-model) | All Rights Reserved | **不复制、不内嵌、不再分发其任何代码或资源**；为定位兼容性问题做过字节码层面的分析；仅在运行时以 Mixin 补返回值，且带版本白名单与 failure-soft |
| [Yes Steve Model 1.20.1 源码](https://github.com/YesSteveModel/YesSteveModel)（分支 `dev/1.20`） | **Apache-2.0** | **参考了其 `client/compat/slashblade` 的语义**（动画名规则、主动画变体），用于对齐 1.20.1 的行为 |
